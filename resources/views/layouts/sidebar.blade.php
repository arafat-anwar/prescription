...
<aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="{{url('dashboard')}}" class="brand-link bg-white">
      	<img src="{{ url('public/system-images/logos/'.systemInformation()->logo) }}" alt="" style="max-height: 30px;">
	</a>
					...
					<ul class="nav nav-treeview">
						...
						<li class="nav-item {{(isset($menu->submenu[0])) ? 'has-treeview' : ''}}  {{$open ? 'menu-open' : ''}}">
							<a @if($menu->route!='#') href="{{url('/'.$module->route.'/'.$menu->route)}}" @else style="cursor: pointer;" @endif class="nav-link {{isset($links[1]) && $links[1] == $menu->route ? 'bg-primary text-white text-bold' : ''}}">
								&nbsp;&nbsp;
								<i class="nav-icon text-white {{(!empty($menu->icon) ? $menu->icon : 'far fa-circle')}}"></i>
								<p class="text-white">
									{{$menu->name}}
							...
							<ul class="nav nav-treeview">
								...
								<li class="nav-item">
									<a href="{{url('/'.$module->route.'/'.$submenu->route)}}" class="nav-link {{isset($links[1]) && $links[1] == $submenu->route ? 'bg-primary text-white text-bold' : ''}}">
										&nbsp;&nbsp;&nbsp;&nbsp;
										<i class="nav-icon text-white {{(!empty($submenu->icon) ? $submenu->icon : 'far fa-dot-circle')}}"></i>
										<p class="text-white">{!! collapseMenuName($submenu->name) !!}</p>
									</a>
								</li>
								@endforeach
							</ul>
